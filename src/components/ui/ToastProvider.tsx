...
const toneStyles: Record<ToastTone, { icon: typeof CheckCircle2; shell: string; iconClass: string }> = {
  success: {
    icon: CheckCircle2,
            ...
            <div className="pointer-events-none fixed inset-x-4 top-4 z-[10100] flex flex-col gap-3 sm:left-auto sm:right-6 sm:w-[min(92vw,380px)]">
              {toasts.map((toast) => {
                ...
                return (
                  <div
                    key={toast.id}
                    className={`pointer-events-auto rounded-2xl border p-4 shadow-xl ring-1 ring-slate-100 ${style.shell}`}
                    role="status"
                    aria-live="polite"
                  >
